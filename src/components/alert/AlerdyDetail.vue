<template>
  <div class="alerdy_detail">
    <HeadTab :numtab="Number($route.query.exit)" />
    <!-- 保单信息 -->
    <div class="confirm_policy c" v-show="isShow">
      <div class="s_m">
        <p>
          <span v-if="!retreatCenter">已分期</span>
          <span v-if="retreatCenter">退保中心</span>
        </p>
        <div class="msg" v-if="!retreatCenter">
          ...
          <div class="w">
            ...
            <div class="r">
              <span>分期期数：</span><input type="text" v-model="phone" disabled>
            </div>
            <div class="r two">
              <span>时间：</span><span class="input">{{ data.createTime | formateDate }}</span>
            </div>
          </div>
          <div class="a"><a @click="check(data.id)">分期计划表</a></div>
        </div>
        <div class="msg" v-if="retreatCenter">
          <div class="warn" style="color:orange;">{{data.remarks}}</div>
          <div class="w">
            <div class="r">
              <span>姓名：</span><input type="text" v-model="data.userName" disabled>
            </div>
            <div class="r">
              <span v-if="data.licenseNumber">车牌号：</span><input v-if="data.licenseNumber" type="text" v-model="data.licenseNumber" disabled>
            </div>
            <div class="r">
              <span v-if="data.certification">车架号：</span><input v-if="data.certification" type="text" v-model="data.certification" disabled>
            </div>
            <div class="r">
              <span>分期金额：</span><input type="text" v-model="data.money" disabled>
            </div>
            <div class="r">
              <span>分期期数：</span><input type="text" v-model="data.monNubmer" disabled>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted () {
    if (this.$route.query.exit === 5) {
      this.retreatCenter = true
      this.getBaodan()
    } else {
      this.getInfoData()
    }
  ...
  methods: {
    getBaodan () {
      // /personal/getPersonalByid?token=18925d61-f039-4104-9ac0-a9b6c4129eba&id=10
      this.$http.get(url + 'personal/getPersonalByid', {
        params: {
        ...
        this.data.money = res.data.data.businessRisks + res.data.data.insuranceRisks + res.data.data.transport
      })
    },
    PersonDetail (data) {
